...
- **#103 / preflight wedge lint.** Tokeniser-wedging synthetic-syntax shapes load cleanly (no parse-table-build failure) and only wedge once a tokenising operation runs against them — recovery (#73) and capture (#79) both kick in too late. `preflight_wedge_check(yaml_text, strict=False)` runs a static lint before ST is asked to compile the YAML, and the same lint runs automatically on every `.sublime-syntax` file under a `temp_packages_link` target (warnings reach the unified log at level WARNING). Initial rule set (intentionally narrow): duplicate cross-scope includes (shape 2) and zero-width-only match paired with push (narrow form of shape 1). Shape 3 from #103 (back-to-back cross-syntax fragment includes under one link) is multi-file and out of single-YAML scope; future expansion when a multi-file lint surface lands.
